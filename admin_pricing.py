...
def _stone_value(stone: dict[str, Any], *keys: str, default: Any = "") -> Any:
    for key in keys:
        value = stone.get(key)
...
def _build_v02_lite_preview(
    ...
    for _, stone in stones.iterrows():
        ...
        row_errors = _format_tuple(legacy_result.get("errors", ()))
        row_warnings = _format_tuple(legacy_result.get("warnings", ()))
        ...
        rows.append(
            {
                ...
                "warnings": _format_tuple(result.warnings),
                "errors": _format_tuple(result.errors),
...
def _v02_lite_summary(preview: pd.DataFrame) -> dict[str, int]:
    if preview.empty:
        return {
            "total": 0,
            "ok": 0,
    ...
    return {
        "total": int(len(preview)),
        "ok": _count_status(statuses, "ok"),
        "needs_review": _count_status(statuses, "needs_review"),
        "blocked": _count_status(statuses, "blocked"),
...
def _render_v02_lite_preview(stones: pd.DataFrame, price_table: pd.DataFrame) -> None:
    ...
    if st.button("Рассчитать v0.2-lite preview", type="secondary"):
        st.session_state["pricing_v02_lite_preview"] = _build_v02_lite_preview(stones, price_table, params)
    ...
    summary = _v02_lite_summary(preview)
...
def render_pricing_tab() -> None:
    ...
    _render_v02_lite_preview(stones, price_table)
